...
# Исправленные и полные URL для всех API
...
def make_image_file(theme):
    # Очищаем тему от спецсимволов и обрезаем строку для стабильности URL
...
def upload_photo_to_vk(path):
    """Загрузка фото полностью через токен ГРУППЫ в указанный альбом"""
    try:
        # 1. Получаем сервер для загрузки в конкретный альбом группы
        server_data = vk("photos.getUploadServer", VK_TOKEN, group_id=int(GROUP_ID), album_id=ALBUM_ID)
        upload_url = server_data["upload_url"]
        
        # 2. Отправляем файл (для альбома поле называется file1)
...
def telegram(msg):
    if TG_TOKEN and TG_CHAT:
        try:
            requests.post(f"{TG_API}{TG_TOKEN}/sendMessage", 
...
def main():
    ...
    # Попытка загрузить сгенерированное фото в альбом
    attachment = ""
    try:
    ...
    # Резервный вариант: если фото не загрузилось, крепим прямую ссылку в текст
    if not attachment:
        try:
            clean_theme = "".join(c for c in theme if c.isalnum() or c.isspace())[:120].strip()
            p = ("Atmospheric cinematic illustration for a russian adventure thriller novel, " 
            ...
            url = (POLLINATIONS_API + requests.utils.quote(p) 
                   + "?width=1200&height=800&nologo=true")
            text += f"\n\n🖼 Иллюстрация: {url}"
