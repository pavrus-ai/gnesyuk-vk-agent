...
log("Версия ℹ️ music-agent v4 (читает music.json, 292 трека)")
...
def generate_text(track, album):
    ...
    co_text = f"В коллаборации с {', '.join(coauthors)}. " if coauthors else ""
    ...
    if url:
        try:
            ...
            if r.headers.get("content-type", "").startswith("image") and len(r.content) > 1000:
                log(f"✅ Обложка альбома: {len(r.content)} байт")
                return r.content
        except Exception as e:
            log(f"⚠️ Обложка не скачалась: {e}")
